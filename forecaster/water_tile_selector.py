...
class WaterTileSelector:
    ...
    def build_manifest_from_tiles(self, tile_records: list[dict]) -> dict:
        """Recompute selection from previously collected per-tile water scenes."""
        return self._build_manifest(deepcopy(tile_records), set())
        ...
        while True:
            ...
            try:
                response = requests.post(
                    TOKEN_URL,
                    data={"grant_type": "client_credentials", "client_id": credentials.client_id, "client_secret": credentials.client_secret},
                    headers={"Content-Type": "application/x-www-form-urlencoded"},
                    timeout=30,
                )
            except requests.RequestException:
                if self._switch_to_next_credentials(0, "token request"):
                    continue
                raise
            if response.status_code == 200:
                token = response.json()["access_token"]
                self._access_tokens[self._credential_index] = token
                print(f"[WaterTileSelector] Using {credentials.label} CDSE credentials ({self._redact_client_id(credentials.client_id)}).")
                return token
            if self._switch_to_next_credentials(response.status_code, "token request"):
                continue
