    ...
    def get_access_token(self):
        ...
        while True:
            ...
            try:
                ...
                print(f"[Statistics] Token request failed with {self._current_credential_label()} credentials: {exc}")
                response = None
            ...
            if self.credential_index + 1 >= len(self.credential_sets):
                ...
                print(f"[Statistics] Token retrieval failed after all credential sets ({status}).")
                return None
            failed_label = self._current_credential_label()
            self.credential_index += 1
            credentials = self.credential_sets[self.credential_index]
            self.client_id = credentials["client_id"]
            self.client_secret = credentials["client_secret"]
            print(f"[Statistics] Token retrieval failed with {failed_label}; trying {credentials.get('label', 'backup')} credentials.")
        ...
        while True:
            ...
            if response.status_code in (403, 429):
                if self._switch_to_next_credentials(response.status_code):
                    # Each configured credential must receive a request. A
                    # credential switch is not a retry of the same request.
                    continue
                exhausted_credential_retries += 1
                if exhausted_credential_retries < retries:
        ...
        for slot in self.slots:
            ...
            if response and response.status_code == 200:
                ...
                self.failures.append({
                    ...
                    "reason": "statistics_request_failed",
                })
    ...
    def get_access_token(self):
        ...
        while True:
            ...
            try:
                ...
                print(f"[ImageCollection] Token request failed with {self._current_credential_label()} credentials: {exc}")
                response = None
            if response is not None and response.status_code == 200:
                print(f"[ImageCollection] Access token retrieved successfully with {self._current_credential_label()} credentials ({self._redact_client_id(self.client_id)}).")
                return response.json()["access_token"]
            if self.credential_index + 1 >= len(self.credential_sets):
                return None
            failed_label = self._current_credential_label()
            ...
            self.client_id = credentials["client_id"]
            self.client_secret = credentials["client_secret"]
            print(f"[ImageCollection] Token retrieval failed with {failed_label}; trying {credentials.get('label', 'backup')} credentials.")
